...
teachers = Blueprint(
    "teachers", __name__, template_folder="templates", url_prefix="/teachers"
)
...
def get_teachers_sorted_by_family_name(teachers) -> Dict:
    """
    Return a dictionary that contains teachers sorted by the first letter
    of the family_name.

    input: list of objects of model Teacher sorted by the family_name
    output: dictionary (string: List[Teacher]), where the key is the first letter
    of family_name and values are objects of model Teacher
    """
    d = {}
    ...
    for teacher in teachers:
        ...
        if (
            first_letter.isalpha() is False
        ):  # some names start with dot '.', or forwardslash '/', (etc.)
            others.append(teacher)
            continue
        first_letter = unidecode.unidecode(
            first_letter
        )  # get rid of diacritics  (Č change to C)
        if string_starts_with_ch(
            teacher.family_name
        ):  # family_name that starts on a "CH" is a special category
            first_letter = "Ch"
