...
def get_highlighted_lesson_ids(user_timetable_id: int) -> Set[int]:
    highlighted_user_timetable_lessons = (
        db.session.query(
            user_timetable_lessons,
        )
        .filter(
            user_timetable_lessons.c.user_timetable_id == user_timetable_id,
            user_timetable_lessons.c.highlighted is True,
        )
        .all()
    )
    return {item._mapping["lesson_id"] for item in highlighted_user_timetable_lessons}
...
def delete_timetable(id_):
    ...
    if len(list(current_user.timetables)) == 0:
        ...
        timetable_to_show_id = current_user.timetables.order_by(UserTimetable.id_)[
            -1
        ].id_
    return jsonify(
        {"next_url": url_for("my_timetable.show_timetable", id_=timetable_to_show_id)}
    )
...
@login_required
@my_timetable.route(
    "/moj-rozvrh/<timetable_id>/lesson/<lesson_id>/<action>", methods=["POST"]
)
def add_or_remove_lesson(timetable_id, lesson_id, action):
    ...
    if action == "add":
        ...
        raise ValueError(
            "Bad route format! There should be either 'add' or 'remove' action in "
            "the URL!"
        )
...
@login_required
@my_timetable.route(
    "/moj-rozvrh/<timetable_id>/subject/<subject_id>/<action>", methods=["POST"]
)
def add_or_remove_subject(timetable_id, subject_id, action):
    """
    Add/Remove subject (with all lessons) to/from user's timetable.
    Return timetable templates (layout & list).
    """
    ut = get_timetable(timetable_id)
...
@login_required
@my_timetable.route(
    "/moj-rozvrh/<timetable_id>/edit-lessons/<action>", methods=["POST"]
)
def edit_lessons(timetable_id, action):
    ...
    if action in ("highlight", "unhighlight"):
        ...
        update_highlighted_statement = (
            user_timetable_lessons.update()
            .where(
