...
from candle.teachers.models import Teacher, teacher_lessons
...
def subject_search_handler():
    """
    Find and return all subjects by subject name or short-code, teachers by name
    (also reversed name) and rooms by room name.

    Request URL format: /subject_search_handler?term=query, where query is a substring
    from a one of above listed choices.
    Respond JSON format: list, where every element is a subject, subject's short-code,
    teacher's name or a room's name.
    """
    query_string = request.args.get("term")
    query_string = query_string.replace(" ", "%")
    query_string = "%{}%".format(query_string)

    # Select unique subject names (each subject must have at least one lesson,
    # so we join with Lessons):
    subjects = (
        Subject.query.join(Lesson)
        .filter(Subject.name.ilike(query_string))
    ...
    subjects_c = (
        Subject.query.join(Lesson)
        .filter(Subject.short_code.ilike(query_string))
        .with_entities(Subject.short_code)
        .distinct()
    ...
    # Search in teachers who have at least one lesson (filter out those who don't
    # have a given_name):
    teachers = (
        Teacher.query.join(teacher_lessons)
        .join(Lesson)
        .filter(Teacher.given_name != "")
        .filter(
            Teacher.fullname.ilike(query_string)
            | Teacher.fullname_reversed.ilike(query_string)
        )
    ...
    rooms = (
        Room.query.join(Lesson)
        .with_entities(Room.name)
        .distinct()
    ...
    for s in subjects_c:
        array.append(
            {"id": s.short_code, "label": s.short_code, "category": "Kódy predmetov"}
        )
    for t in teachers:
...
def lessons_list():
    ...
    if (
        "moj-rozvrh" in pathname_list and current_user.is_authenticated
    ):  # if we are on the user's timetable route:
    ...
    if item_category == "Predmety":
        subjects = (
            Subject.query.filter(Subject.name == item_id).order_by(Subject.name).all()
        )

    elif item_category == "Učitelia":
        subjects = (
            Subject.query.join(Lesson)
            .join(teacher_lessons)
            .join(Teacher)
            .filter(Teacher.slug == item_id)
            .all()
        )
        ...
        subjects = (
            Subject.query.filter(Subject.short_code == item_id)
            .order_by(Subject.short_code)
            .all()
        )

    return render_template(
        "search/subject_search-results.html",
        subjects=subjects,
